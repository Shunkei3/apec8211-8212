...
2. [Regression analysis with R](#hete_r)

+ How to estimate hete-robust standard errors?
+ How to report regression results nicely?
...
???
+ First of all, what is does the problem of heteroskedasticity mean?
  * the variance of the error terms (specifically, the diagonal elements of the variance-covariance matrix of the error term) are  by independent variables. 
+ Does it affect the unbiasedness or consistency property of OLS?
...
???
+ Homoskedasticity is defined as $Var[e|X]=Var[e]=\sigma^2$. That is, the variance of the error terms is independent of $X$ and consistent. 
...
1. This is called <b span style="color:blue">actual, or theoretical variance</b span>, not to be confused with $\hat{\mathbf{V}}_{\hat{\beta}}$, which is the estimator of $\mathbf{V}_{\hat{\beta}}$. 
...
There are various ways to obtain het-robust standard errors (e.g., `coeftest()` from `lmtest` package). In this class, we mainly use `vcovHC()` function from `sandwich` package. `vcovHC()` returns a <span style="color:blue">het-robust variance-covariance matrix</span> of the coefficient estimates in regression models.

<br>

.content-box-green[**Question**]
+ Given a variance covariance matrix, how to get the standard errors of the coefficient estimates?
...
This is one way to report the regression result nicely. 
...
+ For pdf document, use `type=latex`.
+ For html document, use `type=html`.
+ If you want to see the result on R console, use `type=text`.
  ]

.panel[.panel-name[Output: type="text"]

If you use `type="text"` for an html output, you get something like this.
...
If you use `type="html"` for an html output, you get something like this.
...
If you use `type="latex"` for a pdf output, you get something like this.
...
I don't mind if you report your regression result with `summary()` function or `coeftest()` function (from `lmtest` package) in your assignment. 
