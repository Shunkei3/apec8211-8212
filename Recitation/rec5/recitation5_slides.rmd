...
# Outline
1. Quick overview of the big picture
...
<!-- <span font-size = 0.5em><p style="text-align: right;">
[[Appendix B, Step 2: Data Collection, p. B-2](https://learninglink.oup.com/static/5e29bfc369c98b0010b09170/index.html)]
</p></span> -->
...
+ <span style="color:blue">Every statistic (e.g., OLS estimator, t-statistic, standard error) has a sampling distribution</span>
...
<span style="color:blue">The standard error is an *estimator/estimate* of the standard deviation of the sampling distribution.</span>

.content-box-green[**Let's confirm this**]
...
---
# Exercise
+ <span style="color:blue">Generally, unbiasedness does not imply consistency, and vice versa.</span> 
...
+ Estimator 1: Sample mean: $\overline{X_n}=\frac{1}{n}\sum_{i=1}^{n}$
+ Estimator 2: Use only 1st observations $\overline{X_1}$


.content-box-green[**Question**]
+ Show that $\overline{X_1}$ is an unbiased estimator for the population mean $\mu$ 
+ Is $\overline{X_1}$ is a consistent estimator for he population mean $\mu$?

---
class: middle

# Let's check Monte Calro simulation

.medium-code[
.panelset[ 
.panel[.panel-name[Let's try it!]
Goal: Simulate the sampling distribution of $\overline{X_n}$ and $X_1$. 

(1) Generate n=1000 random numbers. 
+ For example, you can use 
  + `rnorm(n, mean = 2, sd = 1)`, population mean is $2$
  +  `rchisq(n, df = 4)`, population mean is $4$
  + `rexp(n, rate = 1)`, population mean is 

(2) Calculate a sample mean. (3)Save the result. (4) Repeat (1)-(3) B=1000 times. (5) Plot the sampling distribution of $\hat{\beta}_1$. 
...
for (i in 1:B){
  ...
  # 2. Obtain sample mean for each estimator 
...
vis_sampling <- 
  ggplot(res_data) +
    geom_histogram(aes(x=x_bar), fill = "green", alpha = 0.5)+
    geom_histogram(aes(x=x_single), fill = "blue", alpha = 0.5)+
    geom_vline(aes(xintercept=mu, color = "red"))+
    labs(title="Sample size 1000")
```
  ]

.panel[.panel-name[Result: n=1000]
```{r}
vis_sampling
```
]
...
.panel[.panel-name[Code 1: n=100, 500, 1000]
```{r, echo=T, warning=F}
# the number of iterations 
B <- 1000
ls_n <- c(100, 500, 1000)
...
vis_sampling <- 
  ggplot(res_dt) +
    geom_histogram(aes(x=x_bar), bins = 500, fill = "green", alpha = 0.5)+
    geom_histogram(aes(x=x_single), bins = 500, fill = "blue", alpha = 0.5)+
    facet_wrap(~ sample_size, ncol=1)+
    xlim(5, 15)
```
  ]
.panel[.panel-name[Result: n=100, 500, 1000]
