...
???
+ Okay, today I want to talk about the Delta method.
+ In this week's assignmnet, you need to use the delta method in the last question. It's a simple,  but it might be confusing if you don't know how to use it. 
+ We learned about the delta method in APEC8211, but as long as I remeber, we did not practice how to apply the delta method. So, in this lab session, we practivce how to use delta method. 

+ More importantly, for statistical inference, delta method is used to derive the asymptotic distribution of the test statistics. So, it's better to understand how to use it. 


+ Also, in this weeks's assignment problem, the delta method is required. 
+ Well, we learned about the delta method in APEC8211, we did not practice how to use it. So, let's practice. 
...
???
+ Suppose that h function is a linear function of a sample moment. In this case, we can directly derive the variance of the plug-in estimator $\hat{beta}$. 
+ Since the inside of the variance is linear, we can use the usaul variance operation and get  $b^2 Var[\hat{\theta}]$. 
...
The long-run, or equibrium, elasticity of $y$ with respect to $x$ is $\theta=\frac{\beta_2}{1-\beta_1}$, estimated as $\theta=\frac{\hat{\beta}_2}{1-\hat{\beta}_1}$. Use the delta method to derive the 
