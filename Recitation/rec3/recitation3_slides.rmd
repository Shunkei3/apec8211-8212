...
+ You know, as Joe told in the class, econometrics cares the error term, and Independence, Mean independence, and Uncorrelatedness are used as a statement of the relationship between the error term and the regressor. 

---
class: middle


Example: A simple linear CEF model
\begin{align*}
Y = \beta_0 + \beta_1 X + e
\end{align*}
...
**What is the logical implication among $X \perp \!\!\! \perp e$, $E[e|X]=0$, and $E[Xe]=0$?**
...
??? 
+ Specifically, the question here is about the distinction among independence, mean independence, and uncorrelatedness, and their logical implication.  

+ Here, Arrow means mathematical logic symbol. For example, if proposition A implies proposition B, then we can write $A \leftarrow B$.
...
???
+ Before we start, I want to mention one thing. 
+ That is, the expectation of the regression error is $0$ as long as an intercept is included in the model. 
+ This is not an assumption at all, this is a sort of a natural result.  
...
Now, $E[u]=E[u-\alpha]=E[u]-\alpha=\alpha-\alpha=0$.
...
+ Independence means statistical independence, right? If $e$ and $X$ are independent, the information about $e$ does not tell anything about the information of the distribution of $X$. 
+ In other words, all the moment of the distribution of $e$ does not change by $X$, and vice versa. 
...
+ and, the uncorrelatedness or orthogonality of $e$ and $X$, simply means there is no linear dependency between $e$ and $X$. 

+ We can see that independence is the strongest condition because it says that all the moments of the distribution of $e$ are irrelevant to $X$. 

+ , and the uncorrelatedness is the weakest condition because it indicates a specific condition, which is linear dependency, between $X$ and $e$.
...
???
+ Generally, this relationship holds. 
+ That is, independence implies mean independence, and mean independence implies uncorrelatedness.  
+ But, the reverse is not true. 

+ Here is the small exercise problem. Let's prove the statement of this direction. 
