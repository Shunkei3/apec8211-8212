...
+ You can skip this question and simply use the result for the next question. 

5. Is $\hat{\sigma}^2$ unbiased estimator for $\sigma^2$? In other words, $E[\hat{\sigma}^2]=\sigma^2$?
6. Propose an unbiased estimator for $\theta$.

???
+ Here is an exercise problem. You might know that variance estimator is biased for the $\sigma^2$, and there is an alternative estimator called bias-correced variance estimator.
+ The logic we gonna practice in this exercise might be helpful for this week's assignment. 
+ Let's do it. 
