...
The difference between $\hat{\sigma}^2$ and $s^2$ directly results in the difference between Het-robust covariance estimators $\hat{\mathbf{V}}_{\hat{\beta}}^{HC0}$ (a.k.a. White covariance matrix estimator) and $\hat{\mathbf{V}}_{\hat{\beta}}^{HC1}$

Both $\hat{\mathbf{V}}_{\hat{\beta}}^{HC0}$ and $\hat{\mathbf{V}}_{\hat{\beta}}^{HC1}$ use the squared residual $\hat{e_i}^2$. 
...
So, HC0 is biased toward $0$. But, they are both asymtotically consistent. 
